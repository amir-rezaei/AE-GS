...
csv = np.genfromtxt('/nas/ei/home/ga87son/py_sim/AE-GS/C16.csv')
#csv = np.genfromtxt('./C16.csv')
C = np.array(csv)
Const = torch.from_numpy(C)  # .unsqueeze(0)

latent_dim = 2  # number of channels (n)
categorical_dim = Const.shape[0]  # 2**k, where k = number of bits
...
test_loader = Data.DataLoader(dataset=datasettest, batch_size=test_num, shuffle=False, num_workers=2)
...
class VAE_gumbel(nn.Module):
    ...
    def forward(self, x):
        ...
        noise = Variable(
            torch.randn(*T.size()) / ((2 * communication_rate * training_signal_noise_ratio) ** 0.5).view(-1, 1))
...
def test(epoch):
    ...
    for i, (data, _) in enumerate(test_loader):
        ...
        H = -torch.sum(qy * log_ratio, dim=-1).detach().numpy()

        _, labels = torch.max(st, 1)
        loss_f = nn.NLLLoss()
        CE = loss_f(torch.log(recon_batch + 1e-20), labels)
        ...
        pdb.set_trace()
        jök
        test_loss += np.sum(temp) * len(data)

    test_loss /= len(test_loader.dataset)
    print('====> Test {} set loss: {:.4f}'.format(epoch,test_loss))


    return -temp, H, H+temp

def run():
    for epoch in range(1, 3):
        plt.show()
        train(epoch)
    MI = np.zeros((test_snr.numpy().shape[0],1000))
    H = np.zeros_like(MI)
    CE = np.zeros_like(MI)
    ...
    plt.title('MI')
    plt.savefig('/nas/ei/home/ga87son/py_sim/AE-GS/MI.png')
    plt.figure()
    plt.plot(test_snr.numpy(), Ha)
    plt.title('Entropy')
    plt.savefig('/nas/ei/home/ga87son/py_sim/AE-GS/En.png')
    plt.figure()
    plt.plot(test_snr.numpy(), CEa)
    plt.title('CE')
    plt.savefig('/nas/ei/home/ga87son/py_sim/AE-GS/CE.png')
...
def loss_function(x_hat, x, p):
    ...
    return (CCE - H)/np.log(2) #torch.log(torch.tensor(2)).long()
